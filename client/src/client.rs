...
impl fmt::Debug for Client {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "blackcoinmore_rpc::Client({:?})", self.client)
    }
}
...
impl RpcApi for Client {
    ...
    fn call<T: for<'a> serde::de::Deserialize<'a>>(
        ...
        if log_enabled!(Debug) {
            debug!(target: "blackcoinmore_rpc", "JSON-RPC request: {} {}", cmd, serde_json::Value::from(args));
        }
...
fn log_response(cmd: &str, resp: &Result<jsonrpc::Response>) {
    if log_enabled!(Warn) || log_enabled!(Debug) || log_enabled!(Trace) {
        match resp {
            Err(ref e) => {
                if log_enabled!(Debug) {
                    debug!(target: "blackcoinmore_rpc", "JSON-RPC failed parsing reply of {}: {:?}", cmd, e);
                }
            }
            Ok(ref resp) => {
                if let Some(ref e) = resp.error {
                    if log_enabled!(Debug) {
                        debug!(target: "blackcoinmore_rpc", "JSON-RPC error for {}: {:?}", cmd, e);
                    }
                    ...
                    trace!(target: "blackcoinmore_rpc", "JSON-RPC response for {}: {}", cmd, result);
                }
            }
        }
    }
